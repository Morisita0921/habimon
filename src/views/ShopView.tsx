...
import { Coins, Check, X, Clock, Package, ShoppingBag, History, Loader2 } from 'lucide-react';
import type { User, ExchangeItem, ExchangeRequest, ExchangeItemCategory } from '../types';
import { CATEGORY_LABELS, CATEGORY_ICONS } from '../data/exchangeItems';
...
export default function ShopView({ user, onUpdateUser: _onUpdateUser, onAddExchangeRequest }: ShopViewProps) {
  ...
  const filteredItems = useMemo(() => {
    if (activeCategory === 'all') return allItems.filter((i) => i.available);
    return allItems.filter((i) => i.available && i.category === activeCategory);
